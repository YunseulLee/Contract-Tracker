    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 14, color: "#8892A0" }}>{c.name}</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
          <StatusBadge urgency={getUrgencyLevel(c)} autoRenew={c.auto_renew} />
          {c.status === "terminated" && <span style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: "#2D1B1B", color: "#FF6B6B", border: "1px solid #8B3A3A" }}>계약 종료</span>}
        </div>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid #1A1F2B", marginBottom: 20 }}>
        ...
        <button onClick={() => setTab("renewal")} style={tabStyle(tab === "renewal")}>
          갱신 관리{c.renewal_status === "pending_review" && <span style={{ marginLeft: 6, width: 8, height: 8, borderRadius: "50%", background: "#FFD700", display: "inline-block" }} />}
        </button>
        ...
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            {[
              ...
              <div key={i} style={{ padding: "10px 14px", background: "#0D1017", borderRadius: 10, border: "1px solid #1A1F2B" }}>
