...
export default function OptionsManager({ studios = [], types = [], contracts = [], onSave, onClose }) {
  ...
  const tagStyle = (inUse) => ({
    ...
    borderRadius: 8, fontSize: 13, background: inUse ? "#1B2333" : "#0D1017",
    color: inUse ? "#6BA3FF" : "#8892A0",
    border: `1px solid ${inUse ? "#2E4A7A" : "#2E3440"}`,
    marginRight: 8, marginBottom: 8,
  });

  const delBtn = (onClick) => (
    <span onClick={onClick} style={{ cursor: "pointer", color: "#FF6B6B", fontSize: 14, lineHeight: 1, marginLeft: 2 }}>×</span>
  );
