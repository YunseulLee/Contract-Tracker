  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ fontSize: 18, fontWeight: 700, color: "#F06B6B", marginBottom: 16 }}>
          계약 삭제
        </div>
        <div style={{ fontSize: 13, color: "#949BAD", marginBottom: 8, lineHeight: 1.6 }}>
          <span style={{ fontWeight: 600, color: "#F0F1F4" }}>
            {contract.vendor} — {contract.name}
          </span>{" "}계약을 삭제하시겠습니까?
        </div>
        <div style={{ fontSize: 12, color: "#636B7E", marginBottom: 20 }}>
          삭제된 계약은 휴지통으로 이동되며 30일 후 완전 삭제됩니다.
        </div>
        <div style={{ fontSize: 12, fontWeight: 600, color: "#F06B6B", marginBottom: 8 }}>
          확인을 위해 아래에 &quot;삭제&quot;를 입력하세요:
        </div>
        <input
          style={{ ...inputStyle, border: `1px solid ${typed === "삭제" ? "#3A8B7A" : "#2B3044"}`, marginBottom: 20 }}
          value={typed} onChange={(e) => setTyped(e.target.value)}
          placeholder="삭제" autoFocus
        />
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{
            padding: "10px 24px", borderRadius: 10, border: "1px solid #2B3044",
            background: "transparent", color: "#949BAD", fontSize: 14, cursor: "pointer",
          }}>취소</button>
          <button
