        ...
        <InputField label="자동갱신">
          <div onClick={() => up("auto_renew", !form.auto_renew)} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", padding: "10px 0" }}>
            <div style={{ width: 44, height: 24, borderRadius: 12, background: form.auto_renew ? "#4A9FD8" : "#2B3044", position: "relative", transition: "background 0.3s" }}>
              <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#F0F1F4", position: "absolute", top: 3, left: form.auto_renew ? 23 : 3, transition: "left 0.3s" }} />
            </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, alignItems: "center" }}>
        <InputField label="분할 결제">
          <div onClick={() => {
            ...
            <div style={{ width: 44, height: 24, borderRadius: 12, background: form.installment_enabled ? "#4A9FD8" : "#2B3044", position: "relative", transition: "background 0.3s" }}>
              <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#F0F1F4", position: "absolute", top: 3, left: form.installment_enabled ? 23 : 3, transition: "left 0.3s" }} />
            </div>
            <span style={{ fontSize: 13, color: "#949BAD" }}>{form.installment_enabled ? "ON" : "OFF (일시불)"}</span>
          </div>
        </InputField>
          ...
          <InputField label="자동 분할">
            <div style={{ display: "flex", gap: 6 }}>
              {[{ l: "2회", n: 2 }, { l: "3회", n: 3 }, { l: "4회(분기)", n: 4 }, { l: "12회(월)", n: 12 }].map(({ l, n }) => (
                ...
                }} style={{ flex: 1, padding: "8px 4px", borderRadius: 6, border: "1px solid #2B3044", background: "#0D0E14", color: "#4A9FD8", fontSize: 11, cursor: "pointer" }}>{l}</button>
              ))}
            </div>
          </InputField>
        )}
      ...
      {form.installment_enabled && form.installment_schedule.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 40px", gap: 8, marginBottom: 8 }}>
            <span style={{ fontSize: 10, color: "#636B7E", padding: "0 4px" }}>회차</span>
            <span style={{ fontSize: 10, color: "#636B7E" }}>결제일</span>
            <span style={{ fontSize: 10, color: "#636B7E" }}>금액</span>
            <span></span>
          </div>
          {form.installment_schedule.map((item, idx) => (
            <div key={idx} style={{ display: "grid", gridTemplateColumns: "60px 1fr 1fr 40px", gap: 8, marginBottom: 6, alignItems: "center" }}>
              <span style={{ fontSize: 13, color: "#949BAD", padding: "0 4px" }}>{item.label}</span>
              ...
              <button onClick={() => { const s = form.installment_schedule.filter((_, i) => i !== idx); up("installment_schedule", s); }} style={{ background: "none", border: "none", color: "#F06B6B", fontSize: 16, cursor: "pointer", padding: 0 }}>×</button>
            </div>
          ))}
          <button onClick={() => up("installment_schedule", [...form.installment_schedule, { date: "", amount: 0, label: `${form.installment_schedule.length + 1}차`, paid: false }])} style={{ padding: "6px 14px", borderRadius: 6, border: "1px dashed #2B3044", background: "transparent", color: "#4A9FD8", fontSize: 12, cursor: "pointer", width: "100%", marginTop: 4 }}>+ 회차 추가</button>
        </div>
      )}

      {/* 담당자 정보 */}
      <div style={{ padding: "14px 0 6px", borderTop: "1px solid #1F2233", marginTop: 8 }}>
        <div style={{ fontSize: 11, fontWeight: 600, color: "#4A9FD8", letterSpacing: "1px", textTransform: "uppercase", marginBottom: 12 }}>👤 담당자 정보</div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
