...
export default function TimelineCalendar({ contracts, onSelectContract }) {
  ...
  const getMonthColor = (key) => {
    ...
    if (items.length === 0) return { bar: "#1E2029", text: "#464B55" };
    ...
    if (minDays <= 30) return { bar: "#EF4444", text: "#F87171" };
    if (minDays <= 60) return { bar: "#EAB308", text: "#FBBF24" };
    return { bar: "#5B8DEF", text: "#5B8DEF" };
  };
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ fontSize: 14, fontWeight: 600, color: "#9BA1AE", marginBottom: 14 }}>📅 만료 타임라인</div>
      <div style={{ background: "#12141A", border: "1px solid #1E2029", borderRadius: 12, padding: "20px 22px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(12, 1fr)", gap: 6 }}>
          {months.map(({ year, month, key }) => {
            ...
            return (
              <div key={key} onClick={() => setSelectedMonth(isSelected ? null : key)} style={{ cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "8px 0", borderRadius: 10, background: isSelected ? "#1E2029" : "transparent", border: isCurrent ? "1px solid #5B8DEF40" : "1px solid transparent", transition: "all 0.2s" }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: isCurrent ? "#EDEEF0" : "#6B7280", letterSpacing: "0.5px", fontFamily: "'Inter', 'Noto Sans KR', sans-serif" }}>{monthNames[month]}</div>
                <div style={{ fontSize: 9, color: "#464B55" }}>{year}</div>
                <div style={{ width: "100%", display: "flex", justifyContent: "center", alignItems: "flex-end", height: 64 }}>
                  <div style={{ width: "60%", height: barHeight, borderRadius: 4, background: count > 0 ? colors.bar : "#1E2029", opacity: count > 0 ? 0.85 : 0.4, transition: "height 0.3s ease" }} />
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: count > 0 ? colors.text : "#2A2D38", minHeight: 18, fontFamily: "'JetBrains Mono', monospace" }}>{count > 0 ? count : ""}</div>
                {isCurrent && <div style={{ width: 4, height: 4, borderRadius: "50%", background: "#5B8DEF" }} />}
              </div>
            );
          })}
        </div>
        ...
        {selectedMonth && selectedItems.length > 0 && (
          <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #1E2029" }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "#9BA1AE", marginBottom: 10 }}>
                ...
                return (
                  <div key={sc.id} onClick={() => onSelectContract(sc)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: uc.bg, border: `1px solid ${uc.border}30`, borderRadius: 10, cursor: "pointer", transition: "background 0.2s" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 13, fontWeight: 600, color: "#EDEEF0" }}>{sc.vendor} — {sc.name}</div>
                        <div style={{ fontSize: 11, color: "#6B7280" }}>{sc.owner_name || sc.studio} · {formatCurrency(sc.annual_cost, sc.currency)}</div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      {events.map((ev, ei) => (
                        <span key={ei} style={{ padding: "3px 10px", borderRadius: 6, fontSize: 10, fontWeight: 600, background: ev.type === "만료" ? "#2D1B1B" : "#1B2333", color: ev.type === "만료" ? "#F87171" : "#5B8DEF", border: `1px solid ${ev.type === "만료" ? "#8B3A3A30" : "#2E4A7A30"}` }}>
        ...
        {selectedMonth && selectedItems.length === 0 && (
          <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid #1E2029", textAlign: "center", padding: "20px 0", color: "#464B55", fontSize: 12 }}>해당 월에 만료/갱신 예정 계약이 없습니다.</div>
        )}
      </div>
    </div>
  );
